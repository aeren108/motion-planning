...
int main(int argc, char *argv[]) {
    ...
    std::vector<vector2f> path_a = form_path(a, vector2f(0.5f, 1.7f));
    std::vector<vector2f> path_b = form_path(b, vector2f(1.f, 1.7f));
    std::vector<vector2f> path_c = form_path(c, vector2f(0.f, 1.7f));
    float totaldist = dist((*g.begin())->x, (*g.begin())->y, path_a[path_a.size() - 1].x, path_a[path_a.size() - 1].y);
    
    cout << "Printing shortest path from 0 to " << SAMPLE_SIZE - 1 << endl;
    cout << "0 -> ";
    for (int i = 0; i < path_a.size(); ++i) {
        cout << "(" << path_a[i].x << "," << path_a[i].y << ") -> ";
        totaldist += dist(path_a[i].x, path_a[i].y, path_a[i-1].x, path_a[i-1].y);
    }
    cout <<"(" << path_a[path_a.size() - 1].x << "," << path_a[path_a.size() - 1].y << ")\n";
    cout << "Milestone count: " << path_a.size() << endl; 
    ...
    cout << "-----------------------\n";
        ...
        for (vector2f v : path_a) a.add_objective(vector2f(v.x, v.y));
